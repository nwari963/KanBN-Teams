...
- [Deliver the Project Wiki MVP](./tickets/012-project-wiki-mvp.md): implementation is committed; Outline side configured (collection `Project Wiki`, key `kan-sync`, Kan env wired, `documents.list` verified 200 from the Kan container); test migration found no supported templates in the current Kan database — template creation, migration apply, and end-to-end checks remain.
