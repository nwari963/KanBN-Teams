...
## Destination
A running, branded **OTSICAL** stack on the studio laptop: Kan (LAN-reachable at http://192.168.1.125:3456), Cal.diy, and Outline (docs layer, docker-compose.outline.yml) in the same Docker estate, teammates onboarded, with small localized tailoring diffs — ready to hand execution to /to-spec when tailoring exceeds config.
...
- [Deliver the Project Wiki MVP](./tickets/012-project-wiki-mvp.md): implementation is committed; test migration found no supported templates in the current Kan database, Outline is back up, and API settings plus end-to-end checks remain.
